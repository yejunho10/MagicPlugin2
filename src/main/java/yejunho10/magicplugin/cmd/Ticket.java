    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (args.length == 0) {
            ...
            try {
                if (sender.isOp()) {
                    if (args[1].isEmpty()) {
                        sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 지급할 플레이어가 입력되지 않았습니다.");
                    } else {
                        if (args[2].isEmpty()) {
                            sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "지급할 티켓 수량이 입력되지 않았습니다.");
                        } else {
            ...
            try {
                if (sender.isOp()) {
                    if (args[1].isEmpty()) {
                        sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 설정할 플레이어가 입력되지 않았습니다.");
                    } else {
                        if (args[2].isEmpty()) {
                            sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "설정할 티켓 수량이 입력되지 않았습니다.");
                        } else {
                            if (!ticket.containsKey(getUUID(args[1]))) {
                                sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "입력한 플레이어가 서버에 접속한 기록이 없습니다.");
                            } else {
            ...
            try {
                if (sender.isOp()) {
                    if (args[1].isEmpty()) {
                        sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 삭제할 플레이어가 입력되지 않았습니다.");
                    } else {
                        if (args[2].isEmpty()) {
                            sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "삭제할 티켓 수량이 입력되지 않았습니다.");
                        } else {
                            if (!ticket.containsKey(getUUID(args[1]))) {
                                sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "입력한 플레이어가 서버에 접속한 기록이 없습니다.");
                            } else {
            ...
            try {
                if (sender.isOp()) {
                    if (args[1].isEmpty()) {
                        sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 초기화할 플레이어가 입력되지 않았습니다.");
                    } else {
                        if (!ticket.containsKey(getUUID(args[1]))) {
                            sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "입력한 플레이어가 서버에 접속한 기록이 없습니다.");
                        } else {
            ...
            try {
                if (args[1].isEmpty()) {
                    p.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 선물할 플레이어가 입력되지 않았습니다.");
                } else {
                    if (args[2].isEmpty()) {
                        p.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 선물할 수량이 입력되지 않았습니다.");
                    } else {
                        if (ticket.containsKey(getUUID(args[1]))) {
                            ...
                            p.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "입력한 플레이어가 서버에 접속한 기록이 없습니다.");
                        }
                    }
                }
            } catch (Exception e) {
            ...
            try {
                if (args[1].isEmpty()) {
                    sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 구매할 수단이 입력되지 않았습니다.");
                } else {
                    if (args[2].isEmpty()) {
                        sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 구매할 수량이 입력되지 않았습니다.");
                    } else {
                        ...
                        if (args[1].equalsIgnoreCase("1")) {
                            ...
                            sender.sendMessage(ChatColor.RED + "[오류] - " + ChatColor.WHITE + "티켓을 구매할 수단이 올바르지 않습니다. (계좌이체=1|문화상품권=2|해피머니=3|컬쳐랜드교환권=4|틴캐시=5|구글기프트카드=6|다이아몬드=7)");
                        }
                    }
                }
            } catch (Exception e) {
            ...
            sender.sendMessage(ChatColor.RED + "/tk buy <수단> <수량> : 티켓을 <수단>을 이용하여 <수량> 만큼 구매합니다.");
